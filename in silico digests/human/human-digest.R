...
library(Biostrings)
library(BSgenome.Hsapiens.UCSC.hg19)  ## use genome of interest
library(plyr)
library(ggplot2)
library(reshape2)
library(scales)
...
for (i in seq_along(Hsapiens)){
  ...
  m <- matchPattern("TCTAGA", Hsapiens[[i]]) ## insert your enzyme's restriction site!!
  starts <- start(gaps(m))
  ends <- end(gaps(m))
  temp_df <- data.frame(start = starts-6, end = ends, chr = seqnames(Hsapiens)[i])
...
ml <- mdf[mdf$width>0 & mdf$width<200001,]
...
write.csv(counts,"XbaI-200000bp-hg19.csv", row.names=FALSE)