...
library(Biostrings)
library(BSgenome.Mmusculus.UCSC.mm10)  ## use genome of interest
library(plyr)
library(ggplot2)
library(reshape2)
library(scales)
...
for (i in seq_along(Mmusculus)){
  ...
  m <- matchPattern("GGATCC", Mmusculus[[i]]) ## insert your enzyme's restriction site!!
  starts <- start(gaps(m))
  ends <- end(gaps(m))
  temp_df <- data.frame(start = starts-6, end = ends, chr = seqnames(Mmusculus)[i])
...
ml <- mdf[mdf$width>0 & mdf$width<45001,]
...
write.table(counts,"BamHI-45000bp.xlsx",sep="\t",row.names=FALSE)
